...
To delete a key we "swap" the item to be deleted with its in-order successor. The successor of any internal node **n** is found in **n**\ |apos|\ s right subtree: it is the first key of the left most leaf node of n's right subtree; for example, given
...
The in order successor of 23 is 27, of 50 is 51, of 60 is 62, and so on\ |ndash|\ all successors of these internal nodes are the first key of the left most child leaf node of the right subtree. "Swap" means we overwrite the item to be deleted with its in-order
successor and then remove the in-order successor from the leaf node. But what if the in order successor is in a 2-node, or similarly what if the key to be deleted is itself a 2-node leaf? To prevent this, as we descend the tree we turn 2-nodes into 3-nodes. The
techniques for converting 2-nodes into 3-nodes are:  
...
Case 2: If each adjacent sibling (there are at most two) has only one item, we know its parent must be a 3- or 4-node (because if it were a 2-node, it will already have been converted to a 3-node). In this case we fuse together the two siblings and bring a
key down from parent, forming a 4-node and shifting all children effected appropriately. For example, if we wish to delete 6 from our first example:
...
Since there a no 3- or 4-node siblings, we fuse the 2-node with 4 into the 2-node with 6, and we bring down 5 from the parent, forming a 4-node leaf below, from which we delete 6:
...
Comments: If the key to be deleted is the largest key, there is no in order successor; however, by ensuring that the node is not a 2-node guarantees (by applying the techniques above) that the tree will remain balanced.

If the key is found in an internal node, the processes of finding its in order successor begins with the subtree rooted at the first child with larger keys. If this immediate child is a 2-node, it must be converted to a
3-node, but this may move the original key either as a result of case 1, barrowing, or case 2, fusing sibling. If this happens, we can simply recurse starting the node where the key was initially found. 
