...
When a temporary is created due to binding to a const reference, it undergoes what is called a "temporary materialization conversion" that converts a prvalue into an xvalue.
...
new. lvalue references are declared using single `&` and rvalue reference are declared using a double `&&`. You can also use "rvalue references as function parameters and return types, as in:
...
The primary use of rvalue references is a funciton parameters and return types. They are not primary used to delcare variables like ri above.

rvalue references only bind to rvalues. This is true even for "rvalue reference to const", for example:
...
We saw previously that binding an "lvalue reference to const" to an rvalue triggers a temporary materialization conversion, resulting in a prvalue being turned into a xvalue. Likewise, binding an "rvalue reference" to an rvalue also triggers
a temporary materialization conversion.

The reason for rvalue references
++++++++++++++++++++++++++++++++

Rvalue reference are in C++11 mainly to implement move operations that avoid unnecessary copying.
