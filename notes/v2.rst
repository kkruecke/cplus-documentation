...
We saw that binding an "lvalue reference to const" to an rvalue triggers a temporary materialization conversion, in which a prvalue is turned into a xvalue (in which the pure rvalue that is not in storage is placed in storage). The temporary materialization conversion also
